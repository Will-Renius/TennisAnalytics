...
for(i in 1:dim(player)[1]){
  if(((player$Outcome[i] == 1) & (player$avgW[i] < player$avgL[i]))){
...
ggplot(data=plotting_dfm, aes(x=Odds,y=preds,color=Surface)) + geom_line()+geom_point(data=nadal,aes(x=Odds,y=Outcome,color=Surface))+facet_wrap(~Round)+labs(title="Nadal Logistic Regression for Outcome of Match",y="Outcome of Match",x="Odds to Win Match")
...
ggplot(data=plotting_dfm, aes(x=Odds,y=preds,color=Surface)) + geom_line()+geom_point(data=federer,aes(x=Odds,y=Outcome,color=Surface))+facet_wrap(~Round)+labs(title="Federer Logistic Regression for Outcome of Match",y="Outcome of Match",x="Odds to Win Match")
```
