...
for(i in 2017:2017){
  ...
  rf.bets <- rf.pred %>% mutate( winnings = payout) %>% dplyr::filter(prediction > 20)
  svm.bets <- svm.pred %>% mutate( winnings = payout) %>% dplyr::filter(prediction > 4)
  reg.bets <- reg.pred %>% mutate( winnings = payout) %>% dplyr::filter(prediction > 20)
...
```{r}
  #method for calculating F1 Statistic
  CM = table(test.data$favorite_won, reg.pred$prediction)
  accuracy = (sum(diag(CM)))/sum(CM)
  print(CM)
  print(accuracy)
  TP = CM[1, 1]
